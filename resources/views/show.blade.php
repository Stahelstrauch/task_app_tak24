...
@section('content')
    ...
    <table class="min-w full border border-gray-300 bg-white rounded shadow">
        ...
        <tr>
            ...
            <td class="border px-4 py-2 flex items-center gap-4">
                ...
                <form action="{{ route('toggle', $todo->id) }}" method="POST">
                @csrf
                @method('PATCH')
                <button type="submit" class="px-3 py-1 rounded bg-blue-300 hover:bg-blue-400 text-white">
                    {{$todo->is_completed ? 'Tegemata' : 'Tehtud'}}</button>
            </form>
            </td>
        </tr>
        <tr>
    ...
    <div class="mt-6">
        ...
        <a href="{{ route('edit', ['task' => $todo->id]) }}" class="inline-block bg-yellow-400 text-black px-4 py-2 rounded hover:bg-yellow-300">Muuda ülesannet</a>

        {{-- Kustutamine on vorm ja sellel peab olema turvanõue csrf --}}
        <form action="{{ route('destroy', $todo->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Oled sa kindel, et soovid selle ülesande kustutada?');">
            @csrf
            @method('DELETE')
